...
#because of a few outliers with population over 10^9 we cannot really see proper distributon of most data (under 10^8) 
#if we want to see more clearly how data is distributed in range to 10^8 we can ignore rare higer walues and make histogram only for them 
```
...
sns.scatterplot(data = df_gapminder, x = 'gdp_cap',
               y = 'life_exp', alpha = .3);
# with higher gdp_cap the low life_exp dissapears and generally goes up
```
...
sns.lineplot(data = df_gapminder,
             x = 'year',
             y = 'life_exp');
# ass years passes life_exp goes up
```
