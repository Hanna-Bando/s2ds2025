...
# Most pokemons have Attack power between 50 to 100 and the rearest type is over 170 attack 
```
...
```python
### Your code here
p = plt.hist(df_pokemon['Defense'], bins=25)
```
...
```python
### Your code here
type1_counts = df_pokemon['Type 1'].value_counts()
...
### Your code here
'''
Normal distribution is when biggest cumulation of data is somwhere in the middle of range values and both sides decrese with quie similar speed
Skewed distribution has definitely more instances of a value on one side, such that the other is difitive minority 
Presence of outlier widens the range of values, creating less focus on actuallu important data, they represent some uniqe cases/anomalities but make analysis harder
Without outliers we can observe how data is observed in general
'''
```
...
''' 
mean will be higher that median
median will be a lower value beacuse high value variables will increase mean
median will not be affecte that much by wef high values
'''
```
...
y = -x * 2 + np.random.normal(loc = 0, scale = 2, size = 100)
plt.scatter(x, y, alpha = .6);
#Lookin only at scatterplot we can say that they seem related when x rises y falls
```
...
plt.ylabel("Attack");
''' 
We learn that in general Pokemons Having Legendary status have on average higher attack power
However this graph doesn't show much else, we cannot see speciffic attack values, how they ere distributet or how many inctances of pokemon each  attac power has
'''
```
