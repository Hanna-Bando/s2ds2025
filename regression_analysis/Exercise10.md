...
```python
# Your code here
sns.scatterplot(data = df_estate, x = "n_convenience", y = "price_twd_msq", alpha = .5);
```
...
```python
# Your code here
sns.scatterplot( data = df_estate, x = "house_age_years", y = "price_twd_msq", alpha = 0.5);
```
...
g.set_axis_labels("house price of unit area", "count")
g.set_titles("House Age Group: {col_name}")
plt.suptitle("Distribution of Price per Square Meter by House Age")
plt.tight_layout()
plt.show()
```
...
```
    ...
    house_age_cat_str                                                       
    0-15               190.0  41.77  14.16   7.6  30.72  42.55  51.68  117.5
    15-30              129.0  32.64  11.40  11.2  23.70  32.90  41.20   59.6
    30-45               95.0  37.65  12.84  12.2  31.20  38.30  42.25   78.3
    

    C:\Users\hazpi\AppData\Local\Temp\ipykernel_22440\3085371179.py:2: FutureWarning: The default of observed=False is deprecated and will be changed to True in a future version of pandas. Pass observed=False to retain current behavior or observed=True to adopt the future default and silence this warning.
      summary_stats = df_estate.groupby('house_age_cat_str')['price_twd_msq'].describe(
...
```
    ...
    Time:                        19:33:28   Log-Likelihood:                -1542.5
...
β₀ = 45.85 -> When dist_to_mrt_m = 0 then predicted price for unit area will equal to 45.85 

β₁ = -0.0073 -> For every meter to mrt price is expected to drop by 0.0073 per unit of area

P>|t| = 0 -> dist_to_mrt_m is a significant factor for price for unit area

R-squared = 0.454 -> Price for unit of area depends in 45.5% on dist_to_mrt_m
...
That there is clearly negative relationship between dist_to_mrt_m and price_twd_msq, there are outliers, especially at sorter distance and in general errors are not constant across all levels which contradict key assumptions of Ordinary Least Squares (homoskedasticity). That can cause problem with proper interpretation/predictions.
...
Point like 270 is an example that even though it has low Leverage its residual is so large that it inflates the model error

Points like 249,116 and 347 have high leverage but moderate residuals they have quite large bubble size, they have disproportionate influence on your regression results

These type of points should be ignored changed or we should implement different model so they don't have as much influence
...
<tr>
  <th>Time:</th>                 <td>19:37:36</td>     <th>  Log-Likelihood:    </th> <td> -1530.2</td>
</tr>
<tr>
...
sns.scatterplot(
    ...
    data=df_estate,
    alpha=0.5,
    hue='house_age_cat_str'
)
plt.title("Price vs. Distance to MRT (Colored by House Age)")
plt.xlabel("dist_to_mrt_m")
...
```
    ...
    Date:                Mon, 02 Jun 2025   Prob (F-statistic):           8.14e-73
    Time:                        19:39:20   Log-Likelihood:                -1497.6
    ...
    log_dist_to_mrt_m    -8.7280      0.414    -21.083      0.000      -9.542      -7.914
...
Model 3 explains about 56% of the variance in housing prices, a clear improvement over Model 2 (48%). And plot is now more linear and scattered which is better, still situation is not perfect because of outliers, but it is significant improvement
...
model3 = smf.ols('price_twd_msq ~ dist_to_mrt_m + house_age_0_15', data=df_estate)
result3 = model3.fit()
print(result3.summary())
```
    ...
    Date:                Mon, 02 Jun 2025   Prob (F-statistic):           4.99e-60
    Time:                        20:00:36   Log-Likelihood:                -1530.2
...
All the VIF values are close to 1, which means there’s almost no collinearity between the variables. It's great because it shows our predictors aren’t messing around with each other, and also the model results are reliable. There’s nothing to worry with multi-collinearity here :))).
...
Our model’s predictions are on average 8.38 off from the real price per square meter on the test set. It shows how much error we might expect if we use the model to predict prices for new houses.
...
```
    ...
    Time:                        23:05:47   Log-Likelihood:                -1525.6
...
We used result2 for diagnostics because it represents our final model with only significant variables included.
...
bp_test = het_breuschpagan(residuals, result2.model.exog)
print("Breusch-Pagan p-value (heteroscedasticity):", bp_test[1])
...
![png](Exercise10_files/Exercise10_67_2.png)
    ...
    Breusch-Pagan p-value (heteroscedasticity): 0.2533256730107636
...
1. Do you understand all numerical measures printed in the SUMMARY of the regression report?
        ...
        Yes, I do. SUMMARY shows things like coefficients (effect of each variable), p-values (if variables are significant), R-squared (how well the model explains the data), and info about the errors.

2. Why do we need a cross-validation?
    
        It helps us check if our model works well on new data, not just the data we used to build it. It helps prevent overfitting ^.^

3. What are the diagnostic plots telling us?
    
        In general they help us spot any problems. They show if our model assumptions are met—like if the errors are random, normal, and have constant spread. 

4. How to compare similar, but competing models?
   
        We can compare them using AIC, BIC, R-squared, or test error (RMSE/MSPE). The model with the better (usually lower) value is the better one.

5. What is VIF telling us?
   
        VIF tells us if our variables are too similar to one another. A high VIF means some variables might be redundant.

6. How to choose best set of predictors for the model?
   
        We use methods like stepwise selection, AIC, or best subset regression to pick variables that are important but don’t make the model very compicated ;)).
