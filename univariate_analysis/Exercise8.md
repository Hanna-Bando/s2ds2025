...
![png](Exercise8_files/Exercise8_53_1.png)
...
The data is almost symmetric with a slightly longer left tail. Most values are concentrated between 7 and 14, with no extreme outliers. The mean being slightly less than the median suggests a small left skew.
...
```python
# skewness > 0 - right-tailed (positive): long tail on the right (larger values are more dispersed). The mean > median.

ratios = df_pokemon.groupby('Type 1')['Speed'].apply(skew)
