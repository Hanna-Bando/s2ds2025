...
```python
cars >> drop(X.weight, X.origin, X.cylinders, X.displacement, X.acceleration,  X.model_year, X.name)
```
...
```python
cars >> select(columns_between('mpg','origin',inclusive=True))
```
...
```python
#cars >> mask((X.origin == 'japan') & (70 <= X.model_year <= 79)) >> pull(X.mpg) >> head(3)

#doesn't  want to work so used pandas instead
cars[
    (cars['origin'] == 'japan') &
    (cars['model_year'] >= 70) &
    (cars['model_year'] <= 79)
...
cars_long = (cars >> gather('variable', 'value', ['mpg', 'horsepower', 'weight', 'acceleration', 'displacement']))
cars_long
```
...
```python
from dfply import spread

# First ensure no duplicates
cars_long = cars_long.drop_duplicates(subset=['cylinders', 'model_year', 'origin', 'name', 'variable'],keep='first')


cars_wide = (cars_long >> spread('variable', 'value'))
cars_wide
```
...
```python
cars >> group_by('origin', 'model_year') >> summarize(mean_mpg = X.mpg.mean())
```
